...
// Period date/label helpers live in financial-period.ts (no Zoho/DB imports)
// so client components can use them without pulling zoho-auth -> pg into
// the browser bundle. Re-exported here so existing server-side callers of
// '@/lib/zoho-reports' keep working unchanged.
import { getFinancialDateRange, getComparisonPeriod, getFinancialPeriodLabel } from './financial-period'
export { getFinancialDateRange, getComparisonPeriod, getFinancialPeriodLabel }
